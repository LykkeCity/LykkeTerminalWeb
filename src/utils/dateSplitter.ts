...
const dateSplitter = () => ({
  bars: (from: number, to: number, resolution: string) => {
    ...
    if (Math.ceil(timePeriodsNumber) > 1) {
      timeFullPeriod = timeDiff / timePeriodsNumber;
      timePartialPeriod = timeDiff % timePeriodsNumber;

      for (let i = 0; i < timePeriodsNumber; i++) {
        timePeriods.push({
          from: (from + timeFullPeriod * i) * 1000,
          to:
            from +
            (timeDiff >= timeFullPeriod
              ? timeFullPeriod * (i + 1)
              : Math.floor(timeFullPeriod * i + timePartialPeriod))
        });
